...
export function executeEffect(object, timelineMetadata, delta) {
  // debugger

  if (
    [EFFECTS.FROM_TO, EFFECTS.GLOBAL_FROM_TO].includes(
      timelineMetadata.effect.name
    )
  ) {
  ...
  } else if ([EFFECTS.GLOBAL_FROM_TO].includes(timelineMetadata.effect.name)) {
    // debugger
  } else {
